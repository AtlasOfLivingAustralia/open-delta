...
public class KeyOutputDirectoryDirective extends AbstractTextDirective {
    ...
    public void process(DeltaContext context, DirectiveArguments directiveArguments) throws Exception {
        ...
        String directiveName = directiveArguments.getFirstArgumentText().trim();
        File datasetDirectory = keyContext.getDataDirectory();
        File directory = Utils.createFileFromPath(directiveName, datasetDirectory);

        if (!directory.exists()) {
            boolean directoryCreated = directory.mkdir();
            if (!directoryCreated) {
                throw DirectiveError.asException(DirectiveError.Error.DIRECTORY_DOES_NOT_EXIST_CANNOT_CREATE, directiveName.length() + 1);
            }
        } else {
        ...
        keyContext.setTypesetFilesOutputDirectory(directory);
    }
