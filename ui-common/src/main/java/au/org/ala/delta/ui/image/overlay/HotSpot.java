...
/**
 * A HotSpot is a selectable region on an Image which is associated with a
 * particular image overlay. Multiple HotSpots can be associated with a single
 * Overlay - see HotSpotGroup.
 */
public abstract class HotSpot extends JPanel implements OverlayLocationProvider, MouseListener {
    ...
    /**
     * If popup is true the HotSpot will draw the outline of the region when the
     * mouse in inside the HotSpot region (or a HotSpot region in the same
     * group.
     */
    ...
    /**
     * True if the region outline should be drawn regardless of the mouse
     * position
     */
    ...
    /**
     * If alwaysDrawHotspot is true the popup will be drawn even if the mouse is
     * not in the hot spot region.
     * 
     * @param drawHotspot
     *            true if the hotspot outline should be drawn regardless of the
    ...
    protected void fireHotSpotDoubleClicked() {
        for (int i = _observers.size() - 1; i >= 0; i--) {
            _observers.get(i).hotSpotDoubleClicked(_overlay);
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (_inHotSpotRegion) {
            // If this was a double click, need to treat the event as
            // "select overlay / click ok"
            boolean implicitOk = e.getClickCount() > 1;

            if (implicitOk) {
                fireHotSpotDoubleClicked();
            } else {
                fireHotSpotSelected();
            }
        }
