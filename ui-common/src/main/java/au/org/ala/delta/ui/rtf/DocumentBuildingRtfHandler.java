...
import au.org.ala.delta.rtf.FontTableParser;
import au.org.ala.delta.rtf.FontTableParser.FontTableHandler;
import au.org.ala.delta.rtf.FontTableParser.RTFFontInfo;
...
public class DocumentBuildingRtfHandler extends RTFHandlerAdapter {
	 ...
	 */
	public DocumentBuildingRtfHandler(DefaultStyledDocument document, int position, JComponent owner, boolean scaleFontsToScreenDPI) {		
		_owner = owner;
		...
		configureAttributeHandlers(scaleFontsToScreenDPI);
		
		addDefaultFontIfMissing(_currentParagraphAttributes);
		addDefaultFontIfMissing(_currentCharacterAttributes);		
	}

	private void configureAttributeHandlers(boolean scaleFonts) {
		...
		_attributeHandlers.put(CharacterAttributeType.FontSize.keyword(), new FontSizeAttributeHandler(scaleFonts));
	...
	private void handleFontTable(String content) {
		FontTableParser.parse(content, new FontTableHandler() {
	...
	public static class FontSizeAttributeHandler implements AttributeHandler {
		...
		private boolean _scale = false;
		
		public FontSizeAttributeHandler(boolean scaleToScreenDPI) {
			_scale = scaleToScreenDPI;
		}
