 ...
 * on demand from disk rather than storing it in memory.
 */
public class SlotFileDataSet extends AbstractObservableDataSet {
    ...
    /**
     * Once there are no more observers of this data set, close the underlying
     * VOP object (which will close files associated with the VOP).
     */
    @Override
    public void close() {
    ...
    public void deleteItem(Item item) {
        synchronized (_vop) {
            ...
            // Remove any references to the item occurring in the directives
            // "files".
    ...
    /**
     * Deletes the supplied character from the SlotFile. Deletion of a character
     * requires a bit of activity.
     * 
     * First we will need to delete any item attributes which involve references
     * to this character, probably after first getting some sort of confirmation
     * from the user that they really wish to proceed. That confirmation is
     * assumed to be handled elsewhere.
     * 
     * Second, we must also delete reference to the character occurring in any
     * of the internal directives "files"
     * 
     * Then we also need to delete any "controlling attributes" based on this
     * character, and update those "controlling attributes" which controlled
     * this character.
     * 
     * And finally we need to delete all the text descriptors "owned" by this
     * character
     * 
    ...
    public void deleteCharacter(Character character) {
        synchronized (_vop) {
            ...
            for (int id : contAttrs) {
                // Do a bit of consistency checking by making sure the
                // controlling attribute had
            ...
            // Then we need to delete all the text descriptors "owned" by this
            // character
            ...
            for (CharTextInfo info : allText) {
                ...
                if (charText.getCharBaseId() == characterId) { // Just being
                                                               // paranoid...
    ...
    // Given a character, fills the vector with the IDs of all items for which
    // an
    ...
    private boolean deleteControlling(int ctlId) {
        ...
        // Get a pointer to the controlling attribute's descriptor, for general
        // use.
        VOControllingDesc ctlBase = (VOControllingDesc) _vop.getDescFromId(ctlId);

        // Next, any characters being controlled by this character will need to
        // have
        // their controlling information updated. That is, the dependencies will
        // need to
        // be removed.
        ...
        if (ctlBase.getNControlled() != 0) { // Didn't successfully remove all
                                             // dependencies!!!
            throw new RuntimeException("Failed to delete all dependencies");
        }

        // Then remove the attribute from the list of attributes "owned" by it's
        // character
    ...
    // will be handled elsewhere. However, we will look for
    ...
    // item descriptions, from internal directives "files", and from image
    // overlays!!!
    protected void doDeleteState(MultiStateCharacter character, int stateNumber) {
        ...
        // This should actually be done by the user before reaching this stage,
        // but we
        // do it here anywhere to guarantee consistency. However, at this stage
        // we would
        // have to second-guess the user about whether any leading or trailing
        // comments
        ...
        // Next, make sure that all references to this state are removed from
        // the directives "files"
        // (The only non-internal directive which currently uses state id is the
        // KEY STATES directive...)
        int dirFileCount = _vop.getDeltaMaster().getNDirFiles();
        for (int i = 1; i <= dirFileCount; ++i) {
            VODirFileDesc dirFile = (VODirFileDesc) _vop.getDescFromId(_vop.getDeltaMaster().uniIdFromDirFileNo(i));
    ...
    /**
     * MultiState types can be changed into other MultiState types. MultiState
     * types can only be changed to a non-MultiState type if the only attribute
     * values that have been coded are comments.
     * 
     * @param character
     *            the Character to be changed.
     * @param newType
     *            the type to change the Character to.
    ...
    /**
     * Numeric types can be changed into other Numeric types. Numeric types can
     * only be changed to a non-numeric type if the only attribute values that
    ...
    /**
     * The SlotFile VOCharBaseDesc only requires minor modification when
     * changing character type, however a new instance of the model Character
     * class needs to be created of the correct type.
     */
    @Override
    public Character changeCharacterType(Character character, CharacterType newType) {
        synchronized (_vop) {
            if (!canChangeCharacterType(character, newType)) {
                throw new IllegalArgumentException("Cannot change Character type from :" + character.getCharacterType() + " to " + newType);
            }
    ...
    /**
     * Returns the directive file with the specified name or null if it does not
     * exist. File path information is not used as part of the matching routine.
     * 
     * @param fileName
     *            the name of the directive file to get.
