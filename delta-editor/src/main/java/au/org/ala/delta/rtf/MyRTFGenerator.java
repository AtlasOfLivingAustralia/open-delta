...
/**
 * Generates an RTF output stream (java.io.OutputStream) from rich text (handed off through a series of LTTextAcceptor calls). Can be used to generate RTF from any object which knows how to write to a
 * text acceptor (e.g., LTAttributedText and LTRTFFilter).
 * 
 * <p>
...
class RTFGenerator extends Object {
	...
	/*
	 * An array of character-keyword pairs. This could be done as a dictionary (and lookup would be quicker), but that would require allocating an object for every character written (slow!).
	 */
	...
	static {
		...
		Dictionary textKeywordDictionary = MyRTFReader.textKeywords;
	...
	public void writeRTFHeader() throws IOException {
		...
		/*
		 * TODO: Should the writer attempt to examine the text it's writing and pick a character set which will most compactly represent the document? (currently the writer always uses the ansi
		 * character set, which is roughly ISO-8859 Latin-1, and uses Unicode escapes for all other characters. However Unicode is a relatively recent addition to RTF, and not all readers will
		 * understand it.)
		 */
		writeBegingroup();
	...
	void writeDocumentProperties(Document doc) throws IOException {
		...
		for (i = 0; i < RTFAttributes.attributes.length; i++) {
			MyRTFAttribute attr = RTFAttributes.attributes[i];
			if (attr.domain() != MyRTFAttribute.D_DOCUMENT)
				continue;
	...
	protected void checkControlWord(MutableAttributeSet currentAttributes, AttributeSet newAttributes, MyRTFAttribute word) throws IOException {
		Object parm;

		if ((parm = attrDiff(currentAttributes, newAttributes, word.swingName(), MagicToken)) != null) {
			if (parm == MagicToken)
				parm = null;
	...
	protected void checkControlWords(MutableAttributeSet currentAttributes, AttributeSet newAttributes, MyRTFAttribute words[], int domain) throws IOException {
		int wordIndex;
		int wordCount = words.length;
		for (wordIndex = 0; wordIndex < wordCount; wordIndex++) {
			MyRTFAttribute attr = words[wordIndex];
	...
	void updateSectionAttributes(MutableAttributeSet current, AttributeSet newAttributes, boolean emitStyleChanges) throws IOException {
		...
		checkControlWords(current, newAttributes, RTFAttributes.attributes, MyRTFAttribute.D_SECTION);
	}
	...
	protected void resetSectionAttributes(MutableAttributeSet currentAttributes) throws IOException {
		...
		for (wordIndex = 0; wordIndex < wordCount; wordIndex++) {
			MyRTFAttribute attr = RTFAttributes.attributes[wordIndex];
	...
	void updateParagraphAttributes(MutableAttributeSet current, AttributeSet newAttributes, boolean emitStyleChanges) throws IOException {
		...
		/*
		 * The only way to get rid of tabs or styles is with the \pard keyword, emitted by resetParagraphAttributes(). Ideally we should avoid emitting \pard if the new paragraph's tabs are a superset
		 * of the old paragraph's tabs.
		 */
		...
		checkControlWords(current, newAttributes, RTFAttributes.attributes, MyRTFAttribute.D_PARAGRAPH);
	...
	void updateCharacterAttributes(MutableAttributeSet current, AttributeSet newAttributes, boolean updateStyleChanges) throws IOException {
		...
		checkControlWords(current, newAttributes, RTFAttributes.attributes, MyRTFAttribute.D_CHARACTER);
	...
	public void writeCharacter(char ch) throws IOException {
		...
		if (b == 0) {
			...
			/*
			 * In some cases it would be reasonable to check to see if the glyph being written out is in the Symbol encoding, and if so, to switch to the Symbol font for this character. TODO.
			 */
			/*
	 ...
	 */
	/*
	 * Not very efficient at all. Could be changed to sort the table for binary search. TODO. (Even though this is inefficient however, writing RTF is still much faster than reading it.)
	 */
	...
	static int[] outputConversionForName(String name) throws IOException {
		char[] table = (char[]) MyRTFReader.getCharacterSet(name);
		return outputConversionFromTranslationTable(table);
	}

	/**
	 * Takes a char and a conversion table (an int[] in the current implementation, but conversion tables should be treated as an opaque type) and returns the corresponding byte value (as an int,
