...
/**
 * Takes a sequence of RTF tokens and text and appends the text described by the RTF to a <code>StyledDocument</code> (the <em>target</em>). The RTF is lexed from the character stream by the
 * <code>RTFParser</code> which is this class's superclass.
 * 
 ...
 */
class MyRTFReader extends MyRTFParser {
	 ...
	 */
	public MyRTFReader(StyledDocument destination) {
		int i;
	...
	/**
	 * Called by the superclass when a new RTF group is begun. This implementation saves the current <code>parserState</code>, and gives the current destination a chance to save its own state.
	 * 
	...
	/**
	 * Called by the superclass when the current RTF group is closed. This restores the parserState saved by <code>begingroup()</code> as well as invoking the endgroup method of the current
	 * destination.
	 * 
	...
	public void close() throws IOException {
		...
		while (docProps.hasMoreElements()) {
			...
			target.putProperty(propName, documentAttributes.getAttribute((String) propName));
		}
	...
	public boolean handleKeyword(String keyword) {
		...
		if (keyword.equals("aftncn") || keyword.equals("aftnsep") || keyword.equals("aftnsepc") || keyword.equals("annotation") || keyword.equals("atnauthor") || keyword.equals("atnicn")
				|| keyword.equals("atnid") || keyword.equals("atnref") || keyword.equals("atntime") || keyword.equals("atrfend") || keyword.equals("atrfstart") || keyword.equals("bkmkend")
				|| keyword.equals("bkmkstart") || keyword.equals("datafield") || keyword.equals("do") || keyword.equals("dptxbxtext") || keyword.equals("falt") || keyword.equals("field")
				|| keyword.equals("file") || keyword.equals("filetbl") || keyword.equals("fname") || keyword.equals("fontemb") || keyword.equals("fontfile") || keyword.equals("footer")
				|| keyword.equals("footerf") || keyword.equals("footerl") || keyword.equals("footerr") || keyword.equals("footnote") || keyword.equals("ftncn") || keyword.equals("ftnsep")
				|| keyword.equals("ftnsepc") || keyword.equals("header") || keyword.equals("headerf") || keyword.equals("headerl") || keyword.equals("headerr") || keyword.equals("keycode")
				|| keyword.equals("nextfile") || keyword.equals("object") || keyword.equals("pict") || keyword.equals("pn") || keyword.equals("pnseclvl") || keyword.equals("pntxtb")
				|| keyword.equals("pntxta") || keyword.equals("revtbl") || keyword.equals("rxe") || keyword.equals("tc") || keyword.equals("template") || keyword.equals("txe") || keyword.equals("xe")) {
	...
	public static Object getCharacterSet(final String name) throws IOException {
		...
		if (set == null) {
			...
			charsetStream = (InputStream) java.security.AccessController.doPrivileged(new java.security.PrivilegedAction() {
				public Object run() {
					return RTFEditorKit.class.getResourceAsStream("charsets/" + name + ".txt");
				}
			});
	...
	class StylesheetDestination extends DiscardingDestination implements Destination {
		...
		public void close() {
			...
			/*
			 * (old debugging code) int i, m; if (characterStyles != null) { m = characterStyles.length; for(i=0;i<m;i++) warnings.println("chrStyle["+i+"]="+characterStyles[i]); } else
			 * warnings.println("No character styles."); if (paragraphStyles != null) { m = paragraphStyles.length; for(i=0;i<m;i++) warnings.println("pgfStyle["+i+"]="+paragraphStyles[i]); } else
			 * warnings.println("No paragraph styles."); if (sectionStyles != null) { m = characterStyles.length; for(i=0;i<m;i++) warnings.println("secStyle["+i+"]="+sectionStyles[i]); } else
			 * warnings.println("No section styles.");
			 */
		}
	...
	/**
	 * RTFReader.TextHandlingDestination is an abstract RTF destination which simply tracks the attributes specified by the RTF control words in internal form and can produce acceptable AttributeSets
	 * for the current character, paragraph, and section attributes. It is up to the subclasses to determine what is done with the actual text.
	 */
	abstract class AttributeTrackingDestination implements Destination {
		...
		public boolean handleKeyword(String keyword) {
...

			{
				...
				if (item != null) {
					MyRTFAttribute attr = (MyRTFAttribute) item;
					boolean ok;

					switch (attr.domain()) {
					case MyRTFAttribute.D_CHARACTER:
						ok = attr.set(characterAttributes);
						break;
					case MyRTFAttribute.D_PARAGRAPH:
						ok = attr.set(paragraphAttributes);
						break;
					case MyRTFAttribute.D_SECTION:
						ok = attr.set(sectionAttributes);
						break;
					case MyRTFAttribute.D_META:
						mockery.backing = parserState;
						ok = attr.set(mockery);
						mockery.backing = null;
						break;
					case MyRTFAttribute.D_DOCUMENT:
		...
		public boolean handleKeyword(String keyword, int parameter) {
			...
			if (keyword.equals("sl")) {
				if (parameter == 1000) { /* magic value! */
					...
					/*
					 * TODO: The RTF sl attribute has special meaning if it's negative. Make sure that SwingText has the same special meaning, or find a way to imitate that. When SwingText handles
					 * this, also recognize the slmult keyword.
					 */
		...
		protected void resetCharacterAttributes() {
			...
			while (attributes.hasMoreElements()) {
				MyRTFAttribute attr = (MyRTFAttribute) attributes.nextElement();
				if (attr.domain() == MyRTFAttribute.D_CHARACTER)
		...
		protected void resetParagraphAttributes() {
			...
			while (attributes.hasMoreElements()) {
				MyRTFAttribute attr = (MyRTFAttribute) attributes.nextElement();
				if (attr.domain() == MyRTFAttribute.D_PARAGRAPH)
					attr.setDefault(characterAttributes);
			}
		}
		...
		protected void resetSectionAttributes() {
			...
			while (attributes.hasMoreElements()) {
				MyRTFAttribute attr = (MyRTFAttribute) attributes.nextElement();
				if (attr.domain() == MyRTFAttribute.D_SECTION)
					attr.setDefault(characterAttributes);
			}
	...
	/**
	 * RTFReader.DocumentDestination is a concrete subclass of TextHandlingDestination which appends the text to the StyledDocument given by the <code>target</code> ivar of the containing RTFReader.
	 */
