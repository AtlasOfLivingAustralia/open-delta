 ...
 * 
 * <p>
 * Most of the functionality of this class would be redundant except that the <code>ByteToChar</code> converters are suddenly private API. Presumably this class will disappear when the API is made
 * public again. (sigh) That will also let us handle multibyte character sets...
 * 
 * <P>
 * A subclass should override at least <code>write(char)</code> and <code>writeSpecial(int)</code>. For efficiency's sake it's a good idea to override <code>write(String)</code> as well. The subclass'
 * initializer may also install appropriate translation and specials tables.
 * 
 * @see OutputStream
 */
abstract class MyAbstractFilter extends OutputStream {
	...
	/**
	 * A convenience method that reads text from a FileInputStream and writes it to the receiver. The format in which the file is read is determined by the concrete subclass of AbstractFilter to which
	 * this method is sent.
	 * <p>
	...
	/**
	 * Hopefully, all subclasses will override this method to accept strings of text, but if they don't, AbstractFilter's implementation will spoon-feed them via <code>write(char)</code>.
	 * 
	 * @param s
