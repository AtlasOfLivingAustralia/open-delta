...
public class TableRowHeader extends JTable implements ReorderableList {
    ...
    public TableRowHeader(EditorViewModel dataModel) {
        ...
        // These lines are required to enable initiating a drag and drop
        // operation
    ...
    /**
     * Registers the action to take when a selection (double click or Enter key)
     * has been made on this list.
     * 
     * @param action
     *            the action that will be invoked on selection.
     */
    ...
    /**
     * Detects double clicks and treats them as a different type of selection
     * event.
     */
    ...
    static class ItemColumnModel extends AbstractTableModel {
        ...
        public ItemColumnModel(EditorViewModel dataSet) {
            ...
            CommentStrippingMode commentStrippingMode = CommentStrippingMode.RETAIN;
            ...
            boolean capitaliseFirstWord = false;
            _formatter = new ItemFormatter(includeNumber, commentStrippingMode, angleBracketHandlingMode, stripRtf, useShortFormOfVariant, capitaliseFirstWord);
        }
