...
public class TreeViewer extends AbstractDeltaView {
	...
	class NumericTextAttributeCellEditor extends AttributeCellEditor implements ValidationListener {
		...
		class EditorPanel extends JPanel {
			...
			/**
			 * Lays out this <code>Container</code>. If editing, the editor will be placed at <code>offset</code> in the x direction and 0 for y.
			 */
			@Override
	...
	/**
	 * Provides support for editors by determining which node has been selected and whether it may be edited
	 */
	abstract class AttributeCellEditor extends DefaultCellEditor {
		...
		/**
		 * Subclasses should override this method to configure the editor appropriately for the supplied node to be edited.
		 * 
		 * @param attribute
	...
	/**
	 * Overrides the editing behaviour of the DefaultTreeCellEditor to allow editing to occur with a single click. (The default is three).
	 */
...
class CharacterTreeModel extends DefaultTreeModel {
	...
	/**
	 * Handles the different values that will be supplied by the different editing components.
	 */
	...
	/**
	 * We notify about changes to text & numeric characters as differing text lengths will invalidate the preferred size of the label that renders the text and this value is cached by the tree UI
	 * delegate. Multistate characters will maintain the same text as the text is specified by the character.
	 */
